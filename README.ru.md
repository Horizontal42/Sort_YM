...
.venv\Scripts\python -m sort_ym lyrics                  # загрузить тексты песен русскоязычных треков (cache/lyrics_text.json), аккаунт не меняется
.venv\Scripts\python -m sort_ym analyze                 # разобрать эти тексты локальной моделью Ollama (cache/lyrics_analysis.json), без интернета, идёт часами, продолжается после обрыва
```
...
- `cache/lyrics_text.json` — тексты песен русскоязычных треков;
- `cache/lyrics_analysis.json` — разбор текста каждого трека локальной моделью.
...
## Разбор текстов (русскоязычные треки)

`lyrics` загружает текст песни, `analyze` читает его локальной моделью и пишет профиль каждого
трека: настроение, темы, эмоциональную арку, ракурс, тон, плюс короткое описание того, о чём
песня и чем она может цеплять.

```
.venv\Scripts\python -m sort_ym lyrics    # тексты песен русскоязычных треков
.venv\Scripts\python -m sort_ym analyze   # разбор локальной Ollama, часы локальных вычислений
.venv\Scripts\python -m sort_ym digest    # out/lyrics_digest.md + агрегат в out/digest.md
```

По умолчанию разбираются только русскоязычные треки — промпт и нарративные поля
(`summary`/`resonance`/`key_line`) написаны на русском и проверены на пилотном прогоне по
русской лирике; качество на других языках не проверялось. Флаг `--all-languages` у `lyrics`
снимает фильтр и загружает/разбирает все треки с доступным текстом независимо от языка.

Что нужно: запущенная локально Ollama с моделью из `config.toml`, блок `[ollama]` (проверить —
`ollama list`). Интернет не используется: `analyze` не обращается к Яндексу вообще.

Скорость — примерно 40 секунд на трек, около трёх часов на ~266 треков. Прогон прерываемый:
результат пишется после каждого трека, Ctrl+C стоит максимум один трек, повторный запуск
продолжает с места обрыва. Треки, упавшие по таймауту, повторяются при следующем запуске.
Увеличение `prompt_version` в `config.toml` обесценивает весь кэш и запускает разбор заново.
