...
Команда полностью офлайн: она не обращается к сети и не требует `.token`, читает только уже
загруженный `cache/`. Нужно, чтобы `report` был запущен хотя бы раз (для `cache/genre_catalog.json`).
