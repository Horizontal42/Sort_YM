...
- **Разбор текстов (`lyrics.py`, `analyze.py`)**: по умолчанию разбираются только русскоязычные
  треки — промпт и нарративные поля (`summary`/`resonance`/`key_line`) написаны на русском и
  проверены на пилотном прогоне по русской лирике; качество на других языках не проверялось.
  `sort_ym lyrics --all-languages` снимает фильтр и загружает/разбирает все треки с доступным
  текстом независимо от определённого языка (`lyrics.ru_lyric_track_ids(...,
  all_languages=True)`). Загрузка текста и анализ — это **две отдельные команды** намеренно: загрузка привязана к сети и устаревшему `track_supplement`,
  ...
  `themes` свободного формата, но ограничены паттерном англ. snake_case (`^[a-z0-9][a-z0-9_-]{2,29}$`) —
